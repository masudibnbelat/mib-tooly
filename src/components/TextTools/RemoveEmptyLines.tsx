...
const MODES: { id: Mode; label: string; description: string }[] = [
  {
    ...
    description: "Remove every empty or whitespace-only line",
  },
  {
    id: "extra",
...
const isWhitespaceOnlyLine = (line: string) => /^[\t ]+$/.test(line);
...
export const RemoveEmptyLinesModal = ({
  ...
  const output = useMemo(() => {
    ...
    switch (mode) {
      ...
      case "blank":
        return lines
          .filter((line) => line === "" || !isWhitespaceOnlyLine(line))
          .join("\n");
  ...
  return (
    <motion.div
      ...
      role="dialog"
      aria-modal="true"
      aria-labelledby="remove-empty-lines-title"
        ...
        <div className="shrink-0 border-b border-(--color-active-border) px-4 pb-4 pt-[max(env(safe-area-inset-top),1rem)] sm:px-6 sm:py-5">
          <div className="flex flex-wrap items-start justify-between gap-4">
            ...
            <motion.button
              ...
              className="flex h-10 w-10 shrink-0 cursor-pointer items-center justify-center rounded-xl border border-red-500 bg-red-500 text-white transition hover:border-red-600 hover:bg-red-600"
            >
              <X className="h-5 w-5" />
            </motion.button>
          </div>
        ...
        <div className="grid min-h-0 flex-1 grid-cols-1 gap-4 overflow-y-auto p-4 sm:p-6 lg:grid-cols-2">
          ...
          <div className="flex min-h-65 min-w-0 flex-col gap-2 lg:min-h-0">
            <div className="flex flex-wrap items-center justify-between gap-2">
              <label
                htmlFor="remove-empty-lines-input"
          ...
          <div className="flex min-h-65 min-w-0 flex-col gap-2 lg:min-h-0">
            <div className="flex flex-wrap items-center justify-between gap-2">
              <label
                htmlFor="remove-empty-lines-output"
                className="text-sm font-medium text-(--color-text)"
              >
                Result
              </label>
        ...
        <div className="shrink-0 border-t border-(--color-active-border) px-4 pb-[max(env(safe-area-inset-bottom),1rem)] pt-4 sm:px-6 sm:py-4">
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div className="flex flex-wrap gap-2">
              ...
              <span className="rounded-xl bg-(--color-active-bg) px-3 py-2 text-sm text-(--color-text)">
                Removed: {removedLines} line{removedLines !== 1 && "s"}
              </span>
            </div>
