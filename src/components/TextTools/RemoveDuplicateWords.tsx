...
const deduplicateTokens = (text: string, caseInsensitive: boolean): string => {
...
export const RemoveDuplicateWordsModal = ({
  ...
  const output = useMemo(() => {
    ...
    switch (mode) {
      case "exact":
        return deduplicateTokens(normalizedInput, false);

      case "case-insensitive":
        return deduplicateTokens(normalizedInput, true);

      case "per-line":
        return normalizedInput
          .split("\n")
          .map((line) => deduplicateTokens(line, true))
          .join("\n");
  ...
  const removedWords = Math.max(inputWordCount - outputWordCount, 0);
  ...
  return (
    <motion.div
      ...
      role="dialog"
      aria-modal="true"
      aria-labelledby="remove-duplicate-words-title"
          ...
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div className="flex min-w-0 items-center gap-3">
              ...
              <div className="min-w-0">
                <h2
                  id="remove-duplicate-words-title"
        ...
        <div className="grid min-h-0 flex-1 grid-cols-1 gap-4 overflow-y-auto p-4 sm:p-6 lg:grid-cols-2">
          ...
          <div className="flex min-h-65 min-w-0 flex-col gap-2 lg:min-h-0">
            <div className="flex flex-wrap items-center justify-between gap-2">
              <label
                htmlFor="remove-duplicate-words-input"
            ...
            <textarea
              id="remove-duplicate-words-input"
              value={input}
          ...
          <div className="flex min-h-65 min-w-0 flex-col gap-2 lg:min-h-0">
            <div className="flex flex-wrap items-center justify-between gap-2">
              <label
                htmlFor="remove-duplicate-words-output"
                className="text-sm font-medium text-(--color-text)"
              >
                Result
              </label>
            ...
            <textarea
              id="remove-duplicate-words-output"
              value={output}
              readOnly
        ...
        <div className="shrink-0 border-t border-(--color-active-border) px-4 pb-[max(env(safe-area-inset-bottom),1rem)] pt-4 sm:px-6 sm:py-4">
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div className="flex flex-wrap gap-2">
              ...
              <span className="rounded-xl bg-(--color-active-bg) px-3 py-2 text-sm text-(--color-text)">
                Removed: {removedWords} word{removedWords !== 1 && "s"}
              </span>
            </div>
